...
class NumpyAppendFile:
    ...
    def initialize(self, array: np.ndarray = None) -> None:
        ...
        if array is not None:
            ...
            # default: Fortran order = False, magic = None (oldest version that works)
            ...
            if new_header.nbytes != self.header_length:
                raise ValueError(f'Cannot expand header size in target file: {self.filename}')
    ...
    def append(self, array: np.ndarray) -> None:
        ...
        if type(array) != np.ndarray:
            if not type(array) == list:
                raise TypeError(f'Input has wrong type: expected type np.ndarray, got {type(array)}')
            array = np.array(array)

        if not (array.flags.c_contiguous or array.flags.f_contiguous):
            # TODO: either make contiguous or throw an exception
            return
        ...
        if array.dtype != self.dtype:
            raise TypeError(f'Array has wrong dtype: expected {self.dtype}, got {array.dtype}')

        if len(array.shape) != len(self.shape):
            raise ValueError(f'Array has wrong number of dimensions: expected {len(self.shape)} != {len(array.shape)}')

        if self.shape[1:] != array.shape[1:]:
            raise ValueError(f'Arrays must match outside the zeroth dimension: expected {self.shape[1:]}, got {array.shape[1:]}')
    ...
    def create_header(self, padding: int = 64) -> memoryview:
        ...
        (prefix_length, ctype, encoding) = header_info[self.magic] # prefix = magic prefix + magic + header size, 10-12 bytes

        buffer.getbuffer()[8:prefix_length] = struct.pack(ctype, int(buffer.getbuffer().nbytes - prefix_length + padding))
        buffer.getbuffer()[-1] = ord(' '.encode(encoding)) # replace current newline character with empty space
        buffer.write(b' ' * 64)
        buffer.getbuffer()[-1] = ord('\n'.encode(encoding)) # insert padded newline character 
